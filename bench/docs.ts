    ...
    blocks: [
      {
        ...
        rows: [
          ...
          [{ zh: "videoTexScale", en: "videoTexScale" }, { zh: "0", en: "0" }, { zh: "视频纹理上传倍率：`0` = 自动（帧率守门按实测帧率往下压），正数 = 固定（1 = 不压最清晰、0.5 = 半幅）。macOS WKWebView 下逐帧 `texImage2D(视频帧)` 要把像素**同步**取回页面进程，代价随像素数线性（全屏视频层 2570×1446 → 16fps，1285×723 → 29fps，上限 30），全屏视频层只有压尺寸这一条路。**显式值优先于自动下坡**（画质页「视频纹理清晰度」就是它）", en: "Video texture upload scale: `0` = automatic (the fps guardian compresses it based on measured fps), a positive number pins it (1 = never compress, 0.5 = half size). Under macOS WKWebView a per-frame `texImage2D(video)` must fetch pixels back into the page process **synchronously**, and the cost scales with pixel count (a full-screen video layer: 2570x1446 -> 16fps, 1285x723 -> 29fps, cap 30), so shrinking is the only lever for full-screen video layers. An explicit value wins over the automatic downgrade" }],
          [{ zh: "autoQuality", en: "autoQuality" }, { zh: "true", en: "true" }, { zh: "自动降档（**只填你没显式指定的键**）：① 探到**软件渲染（无 GPU）**时自动关后处理并把画布 DPR 压到 0.5；② 运行期帧率连续 3 秒低于上限 85% 时把后处理降一档（high→medium→low→off，只降不升，每档间隔 6 秒）；③ 后处理已到底仍不够、且这个场景确实在逐帧传视频纹理时，继续把**视频纹理上传倍率**往下压（1→0.5→0.35：WKWebView 下逐帧 texImage2D 的同步取像素代价随像素数线性，这是唯一的减压手段）。显式的 quality.postProcessing 永远优先；传 false 整体关闭（query ?autoq=0 同效）。实际生效值读 getQuality()", en: "Automatic downgrade (only fills keys you did not set explicitly): (1) on **software rendering (no GPU)** it disables post-processing and caps the canvas DPR at 0.5; (2) at runtime, 3 consecutive seconds below 85% of the fps cap downgrades post-processing one step (high→medium→low→off, down-only, 6s between steps); (3) once post-processing is exhausted and the scene is really uploading video frames, it keeps lowering the **video texture upload scale** (1→0.5→0.35 — under WKWebView a per-frame texImage2D pays a synchronous pixel fetch proportional to pixel count, and this is the only lever left). An explicit quality.postProcessing always wins; pass false to turn it off (?autoq=0 equivalent). Read getQuality() for the effective values" }],
      ...
      },
      {
        ...
        items: [
          ...
          { zh: "④ **视频纹理上传**（WKWebView 专属坑）：WebGL 在 GPU 进程、页面在 WebContent 进程，逐帧 `texImage2D(视频/画布)` 要把像素**同步**取回本进程，代价随该帧像素数线性。实测（macOS 14\" XDR，DPR2，画布 2570×1669，fps 上限 30）：上传 2570×1446 时主线程每帧堵 ~40ms（**16~17fps**，后处理从 medium 降到 off 帧率纹丝不动 —— 这条路径不受后处理档影响）、压到 1285×723 → 29fps、899×506 → 30fps（满帧），同一页面同配置在 Chromium 直接满帧。**根因与解**：WebKit 下 `texImage2D(canvas)` 要把位图**同步**取回页面进程（`RemoteNativeImageProxy::platformImage`，实测 3024 宽上传每帧堵主线程 40~80ms），而 `texImage2D(视频元素)` 走加速面 —— 实测同一场景同尺寸**每帧只要 1~2ms**、满分辨率 3024×1701 也能 30fps 满帧（主线程最长停顿 11~12ms），把 fps 上限调到 60 更是 60fps 满帧。所以改成**直传视频元素优先**，直传失败（个别 WebView）才回退离屏 canvas 中转；另外视频纹理按**图层屏幕足迹**封顶（需要多少传多少），并给帧率守门加了第二段下坡（1 → 0.5 → 0.35，仅在直传不可用、后处理又已到底时才介入）", en: "(4) **video-texture upload** (a WKWebView-specific trap) — WebGL lives in the GPU process while the page lives in WebContent, so a per-frame `texImage2D(video/canvas)` must fetch the pixels back **synchronously**; the cost scales with that frame's pixel count. Measured (macOS 14\" XDR, DPR2, 2570x1669 canvas, 30fps cap): uploading 2570x1446 blocks the main thread ~40ms per frame (**16-17fps**, and downgrading post-processing from medium to off changed nothing — this path ignores the post tier); 1285x723 gives 29fps, 899x506 gives 30fps (full cap), while the same page and config in Chromium hits the cap outright. So video textures are now capped by the **layer's on-screen footprint** (upload what is displayed, skip the oversampling) and the frame-rate guardian gained a second ladder (1 → 0.5 → 0.35, engaged only once post-processing is exhausted and the scene is really uploading video)" }
        ],
      },
    ],
  },
