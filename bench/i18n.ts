...
export const DICT: Record<Lang, Record<string, string>> = {  zh: {
    ...
    "toolbar.dpr": "清晰度",
    "toolbar.dprTip": "渲染分辨率倍率（相对 CSS 像素），同时决定贴图资源倍率：高清 1 / 标准 0.8 / 省电 0.6，auto 按屏幕密度自适应。改档会重建画布并重挂",
    "toolbar.res": "资源",
    "toolbar.resTip": "贴图资源倍率覆盖（默认跟随清晰度档）：native = 不缩，用于和缩放后的画面做逐像素 A/B。改完重挂载",
    "toolbar.resn": "法线",
    "toolbar.resnTip": "只改法线/蒙版的资源倍率（折射与光照对模糊敏感），默认 1。用来拍法线下限的效果对比，改完重挂载",
    "toolbar.resolutionTip": "舞台逻辑分辨率（iframe 视口）",
    "toolbar.volume": "音量",
    "toolbar.aa": "抗锯齿",
    "toolbar.aaTip": "抗锯齿：FXAA=帧末后处理（平滑所有边缘，含贴图边）；MSAA=多重采样（只平滑几何边缘）。全局设置，所有壁纸共用",
    "toolbar.pq": "粒子",
    "toolbar.pqTip": "粒子质量：低/中档按倍率同时缩数量上限与发射率；关=不渲染不推进。全局设置，所有壁纸共用",
    "toolbar.pp": "后处理",
    "toolbar.ppTip": "后处理：低/中档压效果链 FBO 分辨率；关=效果链直通+跳整屏后期层+关 Bloom。全局设置，所有壁纸共用",
    "quality.off": "关",
    "quality.low": "低",
    "quality.medium": "中",
    "quality.high": "高",
    "toolbar.live": "系统实况",
    "toolbar.liveTip": "歌名/进度与音频条走 media-bridge（Node 子进程，系统输出回环采集，不采麦克风）；前台窗口走 AppleScript。换壁纸或勾选后会重挂载",
    "toolbar.localAssets": "原版素材",
    "toolbar.localAssetsTip": "用本机 WE 安装目录的原版内置贴图/法线渲染（拷到 local-assets/ 或用 WE_LOCAL_ASSETS 指路径），而不是仓库的程序化复刻；没装素材时无效果。勾选后重挂载",
    "toolbar.pointerPush": "指针注入",
    "toolbar.pointerPushTip": "模拟桌面壁纸窗口：遮罩挡住原生鼠标事件，坐标改经 __wp.pushPointer 推送 —— 与宿主对接的是同一条通道",
    "toolbar.pause": "暂停",
    "toolbar.resume": "恢复",
    "toolbar.reload": "重挂载",
    "toolbar.release": "释放",
    ...
    "props.pickDir": "选择目录…",
    "props.fileUnset": "未设置",
    "props.fileUploading": "正在导入…",
    "err.wpNotReady": "__wp 尚未就绪（先选一个壁纸并等页面加载完）",
    "err.diagStream": "诊断流断开（dev server 重启？）",
    "err.pickLib": "选择文件夹失败：{msg}",
    "err.pickFile": "选择文件失败：{msg}",
    "err.pickDir": "选择目录失败：{msg}",
    "err.reveal": "打开文件夹失败：{msg}",
    "err.selectFirst": "先选一个壁纸再打开自定义配置",
    "ok.reveal": "已打开文件夹：{id}",
    "log.libLoaded": "壁纸库载入：{n} 项（scene {s} / web {w} / video {v}）",
    "log.mount": "挂载 {id}：?{q}",
    "log.localAssetsOn": "已开启原版素材（本机 WE 内置贴图/法线；没装素材时仍走程序化复刻）",
    "log.localAssetsOff": "已关闭原版素材，使用仓库的程序化内置贴图",
    "log.liveOn": "已开启系统实况（media-bridge 系统音频 + Now Playing + 前台窗口）",
    "log.liveOff": "已关闭系统实况，恢复模拟源",
    "log.pointerPushOn": "已开启指针注入：遮罩屏蔽原生鼠标事件，坐标改经 __wp.pushPointer 推送（模拟桌面壁纸窗口）",
    "log.pointerPushOff": "已关闭指针注入，恢复原生鼠标事件",
    "prompt.libDir": "壁纸库目录",
  },
  en: {
    ...
    "toolbar.dpr": "Clarity",
    "toolbar.dprTip": "Render resolution multiplier (relative to CSS px). Also drives the texture resource scale: High 1 / Standard 0.8 / Eco 0.6; auto follows screen density. Changing it rebuilds the canvas and remounts",
    "toolbar.res": "Resources",
    "toolbar.resTip": "Texture resource-scale override (default follows the clarity tier): native = no downscale, use it for a pixel A/B against the scaled render. Remounts on change",
    "toolbar.resn": "Normals",
    "toolbar.resnTip": "Resource scale for normal/mask textures only (refraction and lighting are blur-sensitive); default 1. Use it to shoot the normal-scale comparison. Remounts on change",
    "toolbar.resolutionTip": "Stage logical resolution (iframe viewport)",
    "toolbar.volume": "Volume",
    "toolbar.aa": "Anti-aliasing",
    "toolbar.aaTip": "Anti-aliasing: FXAA = final-frame post pass (smooths all edges incl. texture alpha); MSAA = multisampled resolve (geometry edges only). Global, applies to all wallpapers",
    "toolbar.pq": "Particles",
    "toolbar.pqTip": "Particle quality: low/medium scale both the count cap and emission rate; off = no simulate, no render. Global, applies to all wallpapers",
    "toolbar.pp": "Post FX",
    "toolbar.ppTip": "Post-processing: low/medium cap effect-chain FBO resolution; off = bypass effect chains, skip fullscreen post layers and Bloom. Global, applies to all wallpapers",
    "quality.off": "Off",
    "quality.low": "Low",
    "quality.medium": "Med",
    "quality.high": "High",
    "toolbar.live": "Live system",
    "toolbar.liveTip": "Title/progress and audio bars via media-bridge (Node subprocess; loopback capture of system output, no mic); front window via AppleScript. Remounts on toggle",
    "toolbar.localAssets": "Stock assets",
    "toolbar.localAssetsTip": "Render with the engine's stock built-in textures/normal maps from this machine (copy them to local-assets/ or point WE_LOCAL_ASSETS at them) instead of the repo's procedural recreations. No effect when absent. Remounts on toggle",
    "toolbar.pointerPush": "Pointer inject",
    "toolbar.pointerPushTip": "Simulates a desktop wallpaper window: a veil blocks native mouse events and coordinates are pushed via __wp.pushPointer — the same channel the native host uses",
    "toolbar.pause": "Pause",
    ...
    "props.pickFile": "Choose file…",
    "props.pickDir": "Choose folder…",
    "props.fileUnset": "Not set",
    "props.fileUploading": "Importing…",
    ...
    "err.pickFile": "Choosing file failed: {msg}",
    "err.pickDir": "Choosing folder failed: {msg}",
    ...
    "log.localAssetsOn": "Stock assets on (local WE built-in textures/normal maps; falls back to procedural when absent)",
    "log.localAssetsOff": "Stock assets off; using the repo's procedural built-in textures",
    "log.liveOn": "Live system on (media-bridge system audio + Now Playing + front window)",
    "log.liveOff": "Live system off; back to simulated sources",
    "log.pointerPushOn": "Pointer injection on: veil blocks native mouse events; coordinates now pushed via __wp.pushPointer (simulates desktop wallpaper window)",
    "log.pointerPushOff": "Pointer injection off; native mouse events restored",
